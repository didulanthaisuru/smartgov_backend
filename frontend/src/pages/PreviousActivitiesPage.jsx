...
const PreviousActivitiesPage = ({ userId="689f4c15c5f5564c686445e2" }) => {
  ...
  // Fetch previous appointments on mount
  useEffect(() => {
    const fetchPreviousAppointments = async () => {
      try {
        ...
        const fetchedActivities = response.data.map((item) => ({
        ...
        alert("Failed to fetch previous activities.");
      }
    };
  ...
  const handleSubmitFeedback = async (activityId) => {
    const activity = activities.find(act => act.id === activityId);
    if (!activity) return;

    const payload = {
      appointment_id: activity.id.toString(),
      rating: activity.rating,
      feedback: activity.feedback
    };

    try {
      const response = await axios.post('http://localhost:8000/api/v1/ratings/', payload);
          ...
          {filteredActivities.map((activity) => (
            <div key={activity.id} className="bg-[#F8CB93] rounded-xl p-6 shadow-md">
              <div className="mb-4 flex items-center justify-between">
                ...
                <p className="text-sm text-black">{activity.is_fully_completed ? "Completed" : "Pending"}</p>
              </div>

              <div className="mb-4">
                <p className="text-sm font-bold text-black">Date: {activity.appointment_date}</p>
              </div>
