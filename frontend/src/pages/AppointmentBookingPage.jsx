...
const AppointmentBookingPage = () => {
  ...
  const [selectedDate, setSelectedDate] = useState(new Date(2025, 0, 7)); // Default to the 7th
  ...
  const handleMonthChange = (event) => {
    setCurrentDate(new Date(currentDate.getFullYear(), event.target.value, 1));
  };
  ...
  const handleConfirmAppointment = async () => {
    ...
    const payload = {
        ...
        appoinment_time: appointmentDateTime.toISOString(), // Using the typo from your schema
    ...
    try {
      ...
      navigate(paymentMethod === "now" ? `/payment/${appointmentId}` : `/confirmation/${appointmentId}`);
    } catch (error) {
      console.error("Failed to confirm appointment:", error);
      alert("There was an error confirming your appointment. Please check the details and try again.");
    } finally {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="border border-gray-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-7 text-center text-xs text-gray-500 mb-3">
              {calendarHeader.map((day) => <div key={day} className="py-1">{day}</div>)}
            </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {timeSlots.map((time) => (
              <button key={time} onClick={() => setSelectedTime(time)} className={`py-2.5 rounded-lg text-sm font-medium transition-all ${selectedTime === time ? "bg-orange-500 text-white shadow-lg" : "bg-orange-100 text-orange-700 hover:bg-orange-200"}`}>{time}</button>
            ))}
          </div>

          {/* --- IMPORTANT NOTICE SECTION (RESTORED) --- */}
          ...
          {/* --- SUMMARY SECTION (RESTORED & DYNAMIC) --- */}
          ...
          <div className="text-center space-y-4">
            <p className="text-xl font-bold text-gray-800">
                Total Payment - Rs. {paymentAmount.toFixed(2)}
            </p>
          ...
          <button
            ...
            className="w-full bg-red-800 text-white py-4 rounded-xl text-lg font-semibold hover:bg-red-900 transition-colors shadow-lg mb-4 disabled:bg-gray-400"
          >
