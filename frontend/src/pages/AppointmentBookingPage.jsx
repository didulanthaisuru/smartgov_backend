...
const VerticalBarChart = ({ data }) => ( 
  <div className="flex justify-around items-end h-20 px-2">
    {data.map((item, index) => (
      <div key={index} className="w-8 rounded-t-sm" style={{ height: `${item.value}%`, backgroundColor: item.color }}></div>
    ))}
  </div> 
);
    ...
    {data.map((item, index) => (
      <div key={index} className="flex items-center gap-2">
        <div className="flex-grow bg-orange-200 rounded-sm h-2.5">
          <div className="bg-orange-400 h-2.5 rounded-sm" style={{ width: `${item.value}%` }}></div>
        </div>
...
const AppointmentBookingPage = () => {
  ...
  const handleTimeSelection = async (time) => {
    console.log(`Time selection clicked: ${time}`);
    ...
    try {
      ...
      const timeMap = {
        "8:30": { hours: 8, minutes: 30 },   // 8:30 AM
        "10:30": { hours: 10, minutes: 30 }, // 10:30 AM  
        "12:30": { hours: 12, minutes: 30 }, // 12:30 PM
        "2:30": { hours: 14, minutes: 30 }   // 2:30 PM → 14:30
      };
      ...
      // Create datetime strings manually to avoid timezone issues
      ...
      // Both should use the same date - the selected date
      const appointmentDateString = `${year}-${month}-${day}T00:00:00.000Z`;
      const appointmentTimeString = `${year}-${month}-${day}T${hours}:${minutes}:00.000Z`;

      const updatePayload = {
        appointment_date: appointmentDateString,  // Same date at 00:00
        appoinment_time: appointmentTimeString,   // Same date with selected time
      };

      console.log('Sending time update payload:', {
        selectedDate: selectedDate.toDateString(),
        selectedTime: time,
        rawSelectedDate: `${year}-${month}-${day}`,
        appointment_date: updatePayload.appointment_date,
        appoinment_time: updatePayload.appoinment_time
      });
      ...
      setSelectedTime("");
      alert(`There was an error selecting this time slot: ${error.response?.data?.detail || error.message}`);
    } finally {
  ...
  const handleConfirmAppointment = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        ...
        {}, // Empty body as backend doesn't expect payload
        {
          headers: {
            'Content-Type': 'application/json',
          }
      ...
      console.log('Appointment confirmed successfully:', confirmationData);
      
      // Prepare appointment details for next page
      ...
      console.log('Navigating to next page with details:', appointmentDetailsForNextPage);
      ...
      console.error("Error details:", error.response?.data || error.message);
      alert(`There was an error confirming your appointment: ${error.response?.data?.detail || error.message}`);
    } finally {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="space-y-4">
            <div className="bg-white border border-gray-100 rounded-lg p-3">
              <h3 className="font-semibold text-sm mb-2 text-gray-700">Visitor traffic By Day</h3>
              <VerticalBarChart data={visitorTrafficByDay} />
            </div>
            <div className="bg-white border border-gray-100 rounded-lg p-3">
              <h3 className="font-semibold text-sm mb-2 text-gray-700">Visitor traffic By Time</h3>
          ...
          <div className="border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <select 
                ...
                className="font-bold text-base text-gray-800 bg-transparent border-none focus:ring-0"
              >
            ...
            <div className="grid grid-cols-7 text-center text-sm gap-1">
              ...
              {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => (
                <button 
                  ...
                  className={`p-2 rounded-full relative text-gray-800 hover:bg-gray-100 ${
                ...
                >
                  {day}
                  {selectedDate.getDate() === day && selectedDate.getMonth() === currentDate.getMonth() && (
                    <Circle className="absolute inset-0 w-full h-full text-orange-500" strokeWidth={1.5} />
                  )}
                </button>
              ))}
            </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-2">
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-semibold text-gray-800">{subServiceName}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-semibold text-gray-800">
                {selectedDate.getFullYear()}/{String(selectedDate.getMonth() + 1).padStart(2, "0")}/{String(selectedDate.getDate()).padStart(2, "0")} - {selectedTime || 'N/A'}
              </span>
            </div>
            <div className="flex justify-between items-center text-sm">
